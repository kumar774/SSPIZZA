  ...
  useEffect(() => {
    if (editId) {
        const fetchRestaurant = async () => {
            const docRef = doc(db, 'restaurants', editId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId) {
        ...
        await setDoc(doc(db, 'restaurants', docRef.id), { ...restaurantData, id: docRef.id, menu: [] }, {merge: true});
        toast.success("New restaurant created!", { id: toastId });
